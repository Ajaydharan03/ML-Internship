...
class Ui_MainWindow(object):
    ...
    def classifyFunction(self):
        try:
            # 1. Check if an image was selected first
            if not hasattr(self, 'file') or not self.file:
                self.textEdit.setText("Error: Browse an image first!")
                print("Error: No image loaded.")
                return

            import os
            # 2. Check if the model files actually exist
            if not os.path.exists('model.json') or not os.path.exists('model.h5'):
                self.textEdit.setText("Error: Model not found. Train first!")
                print("Error: model.json or model.h5 is missing.")
                return
            ...
            print("Result raw array:", result)
            
            label2 = label[result.argmax()]
            print("Final Predicted Label:", label2)
            
            self.textEdit.setText(label2)

        except Exception as e:
            self.textEdit.setText("Error occurred. Check console.")
    ...
    def trainingFunction(self):
        ...
        training_set = train_datagen.flow_from_directory(r'F:\Data Analytics\Day_30\TrainingDataset',
                                                         target_size = (128, 128),
                                                         batch_size = 8,
        ...
        test_set = test_datagen.flow_from_directory(r'F:\Data Analytics\Day_30\TestingDataset',
                                                    target_size = (128, 128),
                                                    batch_size = 8,
                                                    class_mode = 'categorical')
